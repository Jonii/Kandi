...
\emph{Kantalause}. \cite{Topo} Lause 2.9. Olkoon $\X$ joukko, ja $\B \subset \Pow(\X)$. $\B$ on tällöin jonkin topologian kanta, jos ja vain jos seuraavat ehdot pätevät:
...
\begin{equation}
\D = \{U \cap A \mid U \text{ on pisteen }x\text{ ympäristö}\}
\end{equation}
...
Tällainen alkio löytyy muodossa $d_U = A \cap U$. Jos jokin joukon $\D$ alkio $d$ sisältyy alkioon $d_U$, eli pätee $d \supset d_U$, tällöin piste $d$ on muotoa $d = V \cap A$, joten $V \subseteq U$. Täten $n(d) \in V \subset U$. Täten $n$ suppenee kohti pistettä $x$.
...
\bibitem{Topo1}
Jussi Väisälä: Topologia I. Limes ry. 2001
\bibitem{Kelley}
